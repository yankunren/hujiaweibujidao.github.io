...
module AppendFooterFilter
        def append(post)
                ...
                post.content + %Q[<p class='post-footer'>
                        #{pre or "Original link:"}<a href='#{post.full_url}'>#{post.full_url}</a><br/>Written by <a href='#{url}'>#{author}</a>&nbsp;Posted at <a href='#{url}'>#{url}</a><br/>Feel free to read or comment it, and if you want to copy it into your own site, please copy it with its Original Link showed above. Thanks a lot. Hope you enjoy here! ^_^</p>]
        end
end
