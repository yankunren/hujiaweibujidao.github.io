...
[B题:数一的偶像2](http://acdream.info/problem?pid=1670)
...
dp[i][1]=(a+c*0.5)*(dp[i+1][1]+1))+(b+c*0.5)*(dp[i+1][2]+1)
...
这题其实很简单，DP，不过我找到的又不是最好的递推式，不过易于理解，好在卡得不紧张可以打表通过。[标程600ms多一点，我的1000ms多一点]
