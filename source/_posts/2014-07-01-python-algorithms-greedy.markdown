...
上面的分析我们可以看到，通过指向链的方式找到“代表”所花的时间是线性的，这里还可以做些改进。首先，在合并(union)的时候我们让小分支指向大分支，这样更加平衡，平均查找时间肯定有所下降；其次，在查找(find)的时候我们一边查找一边修正经过的点的指向，让它直接指向“代表”，这个怎么做到呢？使用递归就行了，这叫做path compression技术，是这个算法常用的一个技巧。
